'use client';
'use strict';

Object.defineProperty(exports, Symbol.toStringTag, { value: 'Module' });

const jsxRuntime = require('react/jsx-runtime');
const react = require('@zag-js/react');
const factory = require('../factory.cjs');
const forwardRef = require('../forward-ref.cjs');
const accordionContext = require('./accordion-context.cjs');
const accordionItemContext = require('./accordion-item-context.cjs');

const AccordionTrigger = forwardRef.forwardRef((props, ref) => {
  const { getTriggerProps } = accordionContext.useAccordionContext();
  const accordionItem = accordionItemContext.useAccordionItemContext();
  const mergedProps = react.mergeProps(getTriggerProps(accordionItem), props);
  return /* @__PURE__ */ jsxRuntime.jsx(factory.ark.button, { ...mergedProps, ref });
});

exports.AccordionTrigger = AccordionTrigger;
