...
const RangeSliderMarker = forwardRef.forwardRef((props, ref) => {
  const { value, ...spanProps } = props;
  const { getMarkerProps } = rangeSliderContext.useRangeSliderContext();
  const mergedProps = react.mergeProps(getMarkerProps({ value }), spanProps);
