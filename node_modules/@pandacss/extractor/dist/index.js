...
function toBoxNode(value, node, stack) {
  return (0, import_ts_pattern.match)(value).when(isNullish, () => void 0).when(isBoxNode, (value2) => value2).when(isObject, (value2) => {
...
function getDeclarationFor(node, stack, ctx) {
  ...
  let declaration;
  if ((import_ts_morph3.Node.isVariableDeclaration(parent) || import_ts_morph3.Node.isParameterDeclaration(parent) || import_ts_morph3.Node.isFunctionDeclaration(parent) || import_ts_morph3.Node.isBindingElement(parent)) && parent.getNameNode() == node) {
...
var getPropertyName = (property, stack, ctx) => {
  return (0, import_ts_pattern2.match)(property).when(import_ts_morph4.Node.isPropertyAssignment, (property2) => {
    const node = unwrapExpression(property2.getNameNode());
    return (0, import_ts_pattern2.match)(node).when(import_ts_morph4.Node.isIdentifier, (node2) => box.from(node2.getText(), node2, stack)).when(import_ts_morph4.Node.isComputedPropertyName, (node2) => {
      const expression = node2.getExpression();
      stack.push(expression);
      return maybePropName(expression, stack, ctx);
    }).when(
      import_lil_fp.Bool.or(import_ts_morph4.Node.isStringLiteral, import_ts_morph4.Node.isNumericLiteral),
      (node2) => box.from(node2.getLiteralText(), node2, stack)
    ).otherwise(() => void 0);
  }).when(import_ts_morph4.Node.isShorthandPropertyAssignment, (property2) => {
...
function maybeBoxNode(node, stack, ctx, matchProp) {
  ...
  return (0, import_ts_pattern3.match)(node).when(isCached, getCached).when(import_lil_fp2.Bool.or(import_ts_morph6.Node.isStringLiteral, import_ts_morph6.Node.isNoSubstitutionTemplateLiteral), (node2) => {
    const value = trimWhitespace(node2.getLiteralValue());
    return cache(box.literal(value, node2, stack));
  }).when(import_ts_morph6.Node.isObjectLiteralExpression, (node2) => {
    return cache(getObjectLiteralExpressionPropPairs(node2, stack, ctx, matchProp));
  }).when(import_lil_fp2.Bool.or(import_ts_morph6.Node.isTrueLiteral, import_ts_morph6.Node.isFalseLiteral), (node2) => {
    const value = node2.getLiteralValue();
    return cache(box.literal(value, node2, stack));
  }).when(import_ts_morph6.Node.isNumericLiteral, (node2) => {
    const value = node2.getLiteralValue();
    return cache(box.literal(value, node2, stack));
  }).when(import_ts_morph6.Node.isNullLiteral, (node2) => {
    return cache(box.literal(null, node2, stack));
  }).when(import_ts_morph6.Node.isPrefixUnaryExpression, (node2) => {
    ...
    if (!box.isNumberLiteral(boxNode))
      return;
    return cache(operator === import_ts_morph6.ts.SyntaxKind.MinusToken ? box.literal(-Number(boxNode.value), node2, stack) : boxNode);
  }).when(import_ts_morph6.Node.isArrayLiteralExpression, (node2) => {
  ...
  }).when(import_ts_morph6.Node.isIdentifier, (node2) => {
    return (0, import_ts_pattern3.match)(node2.getText()).with("undefined", () => cache(box.literal(void 0, node2, stack))).otherwise(() => cache(maybeIdentifierValue(node2, stack, ctx)));
  }).when(import_ts_morph6.Node.isTemplateHead, (node2) => {
    return cache(box.literal(node2.getLiteralText(), node2, stack));
  }).when(import_ts_morph6.Node.isTemplateExpression, (node2) => {
    const value = maybeTemplateStringValue(node2, stack, ctx);
    return cache(box.literal(value, node2, stack));
  }).when(import_ts_morph6.Node.isTaggedTemplateExpression, (node2) => {
    return cache(maybeBoxNode(node2.getTemplate(), stack, ctx));
  }).when(import_ts_morph6.Node.isElementAccessExpression, (node2) => {
    return cache(getElementAccessedExpressionValue(node2, stack, ctx));
  }).when(import_ts_morph6.Node.isPropertyAccessExpression, (node2) => {
    return cache(getPropertyAccessedExpressionValue(node2, [], stack, ctx));
  }).when(import_ts_morph6.Node.isConditionalExpression, (node2) => {
    ...
    if (box.isEmptyInitializer(condValue))
      return;
    const isFromDefaultBinding = condValue.getStack().some((node3) => import_ts_morph6.Node.isBindingElement(node3));
  ...
  }).when(import_ts_morph6.Node.isCallExpression, (node2) => {
    const value = safeEvaluateNode(node2, stack, ctx);
    if (!value)
      return;
    return cache(box.from(value, node2, stack));
  }).when(import_ts_morph6.Node.isBinaryExpression, (node2) => {
    const operatorKind = node2.getOperatorToken().getKind();
    return (0, import_ts_pattern3.match)(operatorKind).when(isPlusSyntax, () => {
...
var extractCallExpressionArguments = (node, ctx, matchProp = trueFn, matchArg = trueFn) => {
  ...
  return box.array(
    fnArguments.map((argument, index) => {
      ...
      return (0, import_ts_pattern4.match)(argNode).when(
...
// src/extract.ts
var isImportOrExport = import_lil_fp3.Bool.or(import_ts_morph8.Node.isImportDeclaration, import_ts_morph8.Node.isExportDeclaration);
var extract = ({ ast, ...ctx }) => {
  ...
  ast.forEachDescendant((node, traversal) => {
    ...
    if (components) {
      ...
      if (import_ts_morph8.Node.isJsxSpreadAttribute(node)) {
        ...
        const spreadNode = extractJsxSpreadAttributeValues(node, ctx, (0, import_lil_fp3.cast)(matchProp));
        if (!spreadNode)
          return;
        ...
        const processBoxNode = (boxNode) => {
          return (0, import_ts_pattern6.match)(boxNode).when(box.isConditional, (boxNode2) => {
            component.conditionals.push(boxNode2);
          }).when(import_lil_fp3.Bool.or(box.isObject, box.isMap), (boxNode2) => {
            return processObjectLike(boxNode2);
          }).otherwise(import_lil_fp3.noop);
        };
      ...
      if (import_ts_morph8.Node.isJsxAttribute(node)) {
        ...
        (0, import_lil_fp3.pipe)(
          extractJsxAttribute(node, ctx),
          import_lil_fp3.Opt.fromNullable,
          import_lil_fp3.Opt.tap((maybeBox) => {
    ...
    if (functions && import_ts_morph8.Node.isCallExpression(node)) {
      ...
      const nodeList = (0, import_lil_fp3.pipe)(
        boxNodeArray.value,
        import_lil_fp3.Arr.map(
          (boxNode) => (0, import_ts_pattern6.match)(boxNode).when(import_lil_fp3.Bool.or(box.isObject, box.isMap), (boxNode2) => {
  ...
  componentByNode.forEach((parentRef, componentNode2) => {
    ...
    if (!component2)
      return;
    const query = (0, import_lil_fp3.cast)({
      name: parentRef.name,
...
var isObjectLike = import_lil_fp4.Bool.or(box.isObject, box.isMap);
...
// src/unbox.ts
var import_lil_fp5 = require("lil-fp");
...
var getLiteralValue = (node, ctx) => {
  return (0, import_ts_pattern7.match)(node).with(import_ts_pattern7.P.nullish, () => void 0).when(box.isConditional, (node2) => {
    const path = ctx.path;
    ...
    return void 0;
  }).when(import_lil_fp5.Bool.or(box.isLiteral, box.isObject), (node2) => {
    return node2.value;
    ...
    return (0, import_lil_fp5.pipe)(
      import_lil_fp5.Arr.from(node2.value.entries()),
      import_lil_fp5.Arr.map(([key, propNode]) => [
        key,
        getLiteralValue(propNode, Object.assign({}, ctx, { path: ctx.path.concat(key), parent: node2 }))
      ]),
      import_lil_fp5.Arr.filter(([, value]) => isNotNullish(value)),
      Object.fromEntries
    );
...
var unbox = (node, ctx) => {
  ...
  const raw = (0, import_ts_pattern7.match)(node).with(import_ts_pattern7.P.nullish, () => void 0).when(cache.has, cache.get).when(Array.isArray, (node2) => {
    const value = (0, import_lil_fp5.pipe)(
      node2,
      import_lil_fp5.Arr.map((boxNode) => getLiteralValue(boxNode, _ctx)),
      import_lil_fp5.Arr.filter(isNotNullish),
      import_lil_fp5.Arr.head
    );
